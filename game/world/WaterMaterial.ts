...
export class WaterMaterial extends MeshStandardMaterial {
	...
	static {
		this.map = new THREE.TextureLoader().load('/images/Water_1_M_Normal.webp');
		this.map.wrapS = this.map.wrapT = THREE.RepeatWrapping;
	}

	constructor(private readonly waterTexture: THREE.Texture) {
		super({
			map: waterTexture,
			metalnessMap: WaterMaterial.map,
			roughnessMap: WaterMaterial.map,
			normalMap: WaterMaterial.map,
			color: new Color('#0481f6'),
			emissive: new Color('#002a51'),
			transparent: true,
			opacity: 0.5,
			roughness: 0.7,
			metalness: 0.5,
