...
export default function HitToast(data: {
	from: string;
	to: string;
	damage: number;
}) {
	return (
		<motion.div
			...
			className='rounded-md border border-red-700/50 bg-gray-900/50 px-3 py-2 text-lg font-semibold leading-snug text-gray-200'
		>
			<span className='text-toonks-orange'>{data.from}</span>
			<svg
				width='20'
				height='20'
				viewBox='0 0 512 512'
				className='mx-3 -mt-0.5 inline-block fill-current'
			>
				<path
			...
			<span className='text-red-500'>{data.to}</span>
		</motion.div>
	);
}
